...
```{r}
langParGender = cbind( X1LANGST = data$X12LANGST, X2LANGST = data$X12LANGST, X3LANGST = rep(NA, length(data$X1TCHAPP)), X4LANGST = data$X4LANGST)
langParGender = data.frame(langParGender); head(langParGender)
langParGender = apply(langParGender, 2, function(x){ifelse(x == -9, NA, x)})
langParGender = as.data.frame(langParGender)

langParGender = ifelse(is.na(langParGender), NA, ifelse(langParGender == 1, 1,0))
langParGender = as.data.frame(langParGender)
head(langParGender)


childGender =cbind(X1_CHSEX_R = data$X_CHSEX_R,  X2_CHSEX_R = data$X_CHSEX_R,  X3_CHSEX_R = data$X_CHSEX_R,  X4_CHSEX_R = data$X_CHSEX_R)
...
childEth = cbind(X_RACETHP_R = data$X_RACETHP_R)
...
data1 = cbind(data1, langParGender, childGender, childEth, parEth)
```
