...
ECLSK1 = cbind(ECLSK1, X1_CHSEX_R, X1_HISP_R, X1_BLACK_R, X1_ASIAN_R, X1_AMINAN_R, X1_MULTR_R, X1PAR1RAC); head(ECLSK1)
...
ECLSK2 = cbind(ECLSK2, X1_CHSEX_R, X1_HISP_R, X1_BLACK_R, X1_ASIAN_R, X1_AMINAN_R, X1_MULTR_R, X1PAR1RAC); head(ECLSK2)
...
ECLSK3 = cbind(ECLSK3, X1_CHSEX_R, X1_HISP_R, X1_BLACK_R, X1_ASIAN_R, X1_AMINAN_R, X1_MULTR_R, X1PAR1RAC); head(ECLSK3)
...
ECLSK4 = cbind(ECLSK4, X1_CHSEX_R, X1_HISP_R, X1_BLACK_R, X1_ASIAN_R, X1_AMINAN_R, X1_MULTR_R, X1PAR1RAC); head(ECLSK4)
...
ECLSK5 = cbind(ECLSK5, X1_CHSEX_R, X1_HISP_R, X1_BLACK_R, X1_ASIAN_R, X1_AMINAN_R, X1_MULTR_R, X1PAR1RAC); head(ECLSK5)
```
...
head(ECLSK1)
```
Equations 1.1 through 1.3 describe the mutlilevel levels model.  First there is level one, which has a intercept \beta_{0j} and the one of the variables of interest \beta_{1j} which is the coefficent for the time variable.  The i's in time indcate each time point, which are nested in each person j where j can vary across each person's intercept and slope.  Beta1j interacts with Beta2j which is the alternative to assigned school variable, which can vary by person over time.  This interaction is the variable of interest and represents whether or not students in alternatives to assigned public schools have different scores on teacher's reported self control over time.  Then there is \beta_{xj}, which represents a vector of time varying covariates found int Table (insert number).  Finally there is the indiviudal error term e_{ij}, which is the difference between the predicated and observed value for each person over each time point.  

Next, the intercept is broken down into the different factors influencing its value for each student.  \beta_{0j} represents the average intercept value and \beta_{0x} represents the level time invarient covariates including the in the model, which are also found in Table (insert number).  Then there is the individual variation that each person has from the average intercept value represented by u0j.  

Finally, there is the random slope, which allows each indiviudal i to have their trajectory or slope over self control over the variable time j.  X represents each of the included level one beta's that can vary over time.  Each beta can vary over time, because it will have a unique error term uxj that can over the variable time. 

Have x represent the following level one covariates X1TCHAPP, X1TCHPER, X1TCHEXT, X1TCHINT, X1RTHET, X1MTHET, X1BMI, X1PUBPRI
Have x represent the following level two variables: X1_CHSEX_R, X1_HISP_R, X1_BLACK_R, X1_ASIAN_R, X1_AMINAN_R, X1_MULTR_R, X1PAR1RAC

$$ Level~1:~~~{y_{ij} = \beta_{0j} + \beta_{1j}(time_{ij})*(Alternative_{ij}) + \beta_{xj} + e_{ij}}~~~ (1.1)$$
$$ Level~2~Intercept:~~~{\beta_{0j} = \gamma_{00} + \gamma_{0x} + u_{0j}} ~~~ (1.2)$$
$$ Level~2~Slope:~~~{\beta_{xj} = \gamma_{x0} + u_{xj}} ~~~ (1.2)$$
$$Mixed~model: ~~~{y_{ij} = \gamma_{00}+\gamma_{10}(Time_{ij})*(Alternative_{j}) + \gamma_{x0}*(Level~One~Covariates)+ \gamma_{0x}(Level~Two~Covariates) + u_{xj} + u_{0j} + e_{ij}} ~~~(1.3)$$
...
Finally, I collect the means and standard deviations for each of the included variables that I collected earlier and use the same process as with the parameter estimates and standard errors to average them.
```{r}
library(Amelia)
