...
Correct id's here going from 1 to end.
...
head(ECLSK1)
```
Now we need to do the multilevel analysis 

Compare full model with all covariates with no covarites should be the same

Do nesting comparisions with null, random intercepts, then random for both intercepts and slopes

Graph: (All get means across time for variables).  
```{r}
library(nlme)

model1 = lme(fixed = X1TCHCON ~ time*X1PUBPRI  +X1MTHET +  X1TCHAPP + X1TCHPER + X1TCHEXT + X1TCHAPP+X1TCHPER +X1TCHEXT+ X1TCHINT+X1MTHET+X1BMI, random = ~1 | id, data = ECLSK1)
summary(model1)

model1 = lme(fixed = X1TCHCON ~ time*X1PUBPRI  +X1MTHET +  X1TCHAPP + X1TCHPER + X1TCHEXT + X1TCHAPP+X1TCHPER +X1TCHEXT+ X1TCHINT+X1MTHET+X1BMI, random = ~time | id, data = ECLSK1)
summary(model1)
```
