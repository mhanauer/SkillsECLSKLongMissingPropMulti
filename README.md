...
$$Mixed~model: ~~~{y_{ij} = \gamma_{00}+ \gamma_{0Q}(Q_{j}) +  \gamma_{10}(Time_{j}) + \gamma_{11}(Time_{ij})(Private_{j}) + \gamma_{x0}(X_{ij}) +u_{ij}(Time_{ij}) + u_{1j} + u_{xj}(X_{ij}) + u_{0j} + e_{ij}} ~~~(1.5)$$
